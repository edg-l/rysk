...
fn an_interrupt_that_is_pending_but_not_enabled_is_not_taken() {
    ...
    let machine = prog(&[addi(A0, ZERO, 1)])
        .csr(MIP, SSIP)
        .csr(MSTATUS, 1 << MSTATUS_MIE)
        .expect(Exception::IllegalInstruction(0));
    assert_eq!(machine.reg(A0), 1, "the program ran to its own end");
}

#[test]
fn an_interrupt_that_is_enabled_and_pending_enters_the_handler() {
    let [off, uninstall] = quiesce(true);
    let machine = prog(&[addi(A0, ZERO, 1), off, uninstall, addi(A1, ZERO, 1)])
        .csr(MIP, SSIP)
        .csr(MIE, SSIE)
        .csr(MSTATUS, 1 << MSTATUS_MIE)
        .csr(MTVEC, DRAM_BASE + 4)
        .expect(Exception::IllegalInstruction(0));
    assert_eq!(machine.reg(A0), 0, "taken before the first instruction ran");
    assert_eq!(machine.reg(A1), 1, "and the handler ran instead");
    assert_eq!(
        machine.csr(MCAUSE),
...
fn a_machine_interrupt_is_not_held_off_by_a_less_privileged_mode() {
    ...
    let machine = prog(&[addi(A0, ZERO, 1), off, uninstall, addi(A1, ZERO, 1)])
        ...
        .csr(MTVEC, DRAM_BASE + 4)
        .expect(Exception::IllegalInstruction(0));
    assert_eq!(machine.reg(A1), 1, "the machine handler ran anyway");
...
fn a_vectored_handler_spreads_the_interrupts_out() {
    ...
    let machine = prog(&[addi(A1, ZERO, 1), off, uninstall, addi(A0, ZERO, 1)])
        .csr(MIP, SSIP)
        .csr(MIE, SSIE)
        .csr(MSTATUS, 1 << MSTATUS_MIE)
    ...
    assert_eq!(machine.reg(A1), 0, "and it was not entered at the base");
}

#[test]
fn the_timer_interrupt_follows_mtime_past_mtimecmp() {
    let machine = timed(&[sd(T1, T0, 0), csrrs(A0, MIP as u32, ZERO)])
        .reg(T1, 0)
        .run();
    assert_ne!(
        machine.reg(A0) & MTIP,
        0,
        "a deadline in the past is pending"
    );
    ...
    assert_eq!(
        machine.reg(A0) & MTIP,
        0,
        "and one that never arrives is not"
    );
}
...
fn software_cannot_clear_a_pending_bit_a_device_is_driving() {
    ...
    assert_ne!(
        machine.reg(A0) & MTIP,
        0,
        "a write cannot argue with a wire"
    );
}
...
fn a_timer_that_is_armed_and_enabled_eventually_fires() {
    ...
    assert_eq!(
        machine.reg(A1),
        0,
        "which had not run when the trap was taken"
    );
    assert_eq!(
        machine.csr(MCAUSE),
        INTERRUPT | Interrupt::MachineTimer as u64
    );
    assert_eq!(
        machine.csr(MEPC),
        DRAM_BASE + 8,
...
fn wfi_retires_at_once_when_something_is_already_pending() {
    let machine = prog(&[wfi(), addi(A0, ZERO, 1)])
        .csr(MIP, SSIP)
        .csr(MIE, SSIE)
        .run();
    assert_eq!(machine.reg(A0), 1, "it did not stall");
}
...
fn wfi_waits_for_an_interrupt_that_has_not_arrived_yet() {
    ...
    let machine = timed(&[sd(T1, T0, 0), wfi(), csrrs(A0, MIP as u32, ZERO)])
        .reg(T1, SOON)
        .csr(MIE, MTIE)
        .run();
    assert_ne!(
...
fn an_interrupt_delegated_to_supervisor_mode_enters_stvec() {
    ...
    let machine = prog(&[addi(A0, ZERO, 1), off, uninstall, addi(A1, ZERO, 1)])
        ...
        .expect(Exception::IllegalInstruction(0));
    assert_eq!(machine.reg(A1), 1, "the supervisor handler ran");
    assert_eq!(
        machine.mode(),
        Mode::Supervisor,
        "and stayed in supervisor mode"
    );
    assert_eq!(
        machine.csr(SCAUSE),
        INTERRUPT | Interrupt::SupervisorSoftware as u64
    );
    assert_eq!(machine.csr(MCAUSE), 0, "machine mode was never told");
}

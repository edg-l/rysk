...
fn stores_and_loads_round_trip_every_width() {
    ...
    .run();
    assert_eq!(machine.reg(T2), 0x8899_aabb_ccdd_eeff);
    assert_eq!(machine.reg(T3), 0xccdd_eeff);
    assert_eq!(machine.reg(T4), 0xeeff);
    assert_eq!(machine.reg(T5), 0xff);
}
...
fn a_doubleword_keeps_its_top_bits() {
    ...
    let machine = prog(&[sd(T1, T0, 0), ld(T2, T0, 0)])
        .reg(T0, SCRATCH)
        .reg(T1, u64::MAX)
        .run();
...
fn a_doubleword_lands_in_the_right_byte_lanes() {
    let machine = prog(&[sd(T1, T0, 0)])
        ...
        .run();
    for byte in 0..8u64 {
        assert_eq!(machine.load(SCRATCH + byte, 1), byte + 1, "byte {byte}");
    }
}
...
fn narrow_loads_sign_extend_and_their_unsigned_forms_do_not() {
    ...
    .run();
    assert_eq!(machine.reg(T2), u64::MAX);
    assert_eq!(machine.reg(T3), 0xff);
    assert_eq!(machine.reg(T4), u64::MAX);
    assert_eq!(machine.reg(T5), 0xffff);
    assert_eq!(machine.reg(T6), u64::MAX);
}

#[test]
fn a_negative_offset_addresses_below_the_base() {
    let machine = prog(&[sd(T1, T0, -8), ld(T2, T0, -8)])
        .reg(T0, SCRATCH)
        .reg(T1, 0x1234)
        .run();
    assert_eq!(machine.reg(T2), 0x1234);
    assert_eq!(machine.load(SCRATCH - 8, 8), 0x1234);
}

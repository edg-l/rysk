...
fn mret_returns_to_the_mode_stacked_in_mpp() {
    for mode in [Mode::User, Mode::Supervisor, Mode::Machine] {
        let machine = prog(&[mret(), addi(A0, ZERO, 1)])
            .csr(MSTATUS, mpp(mode) | (1 << MSTATUS_MPIE))
            .csr(MEPC, DRAM_BASE + 4)
            .run();
        assert_eq!(machine.mode(), mode, "mret entered the mode mpp named");
        assert_eq!(machine.reg(A0), 1, "and carried on at mepc");
        assert_eq!(
            machine.csr(MSTATUS) & MSTATUS_MPP,
            0,
...
fn sret_returns_to_the_mode_stacked_in_spp() {
    for (spp, mode) in [(0, Mode::User), (1, Mode::Supervisor)] {
        let machine = prog(&[sret(), addi(A0, ZERO, 1)])
            .mode(Mode::Supervisor)
            .csr(MSTATUS, (spp << MSTATUS_SPP) | (1 << MSTATUS_SPIE))
            .csr(SEPC, DRAM_BASE + 4)
            .run();
        assert_eq!(machine.mode(), mode, "sret entered the mode spp named");
        assert_eq!(machine.reg(A0), 1, "and carried on at sepc");
        assert_eq!(
            machine.csr(MSTATUS) & (1 << MSTATUS_SPP),
            0,
            "spp is left at user"
        );
        assert_ne!(
            machine.csr(MSTATUS) & (1 << MSTATUS_SIE),
            0,
            "spie moved into sie"
        );
        assert_ne!(
            machine.csr(MSTATUS) & (1 << MSTATUS_SPIE),
            0,
...
fn ecall_raises_the_cause_that_names_the_mode_it_came_from() {
    for mode in [Mode::User, Mode::Supervisor, Mode::Machine] {
        let machine = prog(&[ecall()])
            .mode(mode)
            .expect(Exception::EnvironmentCall(mode));
        assert_eq!(
            machine.mode(),
            mode,
            "nothing took the trap, so nothing moved"
        );
    }
...
fn a_trap_records_the_mode_it_came_from_and_enters_machine_mode() {
    ...
    assert_eq!(
        machine.mode(),
        Mode::Machine,
        "the handler runs in machine mode"
    );
    assert_eq!(
        machine.csr(MSTATUS) & MSTATUS_MPP,
        mpp(Mode::User),
    ...
    assert_eq!(machine.csr(MCAUSE), 8, "environment call from u-mode");
    assert_eq!(machine.csr(MEPC), DRAM_BASE, "at the ecall, not past it");
    assert_eq!(machine.reg(A0), 1, "the handler ran");
}
...
fn a_delegated_exception_enters_the_supervisor_handler() {
    ...
    let machine = prog(&[ecall(), csrrw(ZERO, 0x105, ZERO), addi(A0, ZERO, 1)])
        .mode(Mode::User)
        .csr(MEDELEG, 1 << 8)
    ...
    assert_eq!(
        machine.mode(),
        Mode::Supervisor,
        "the handler runs in s-mode"
    );
    assert_eq!(machine.csr(SCAUSE), 8, "scause took the cause");
    assert_eq!(machine.csr(SEPC), DRAM_BASE, "and sepc the address");
    assert_eq!(machine.csr(STVAL), 0, "an environment call owes no value");
    assert_eq!(
        machine.csr(MCAUSE),
        0,
        "the machine registers are untouched"
    );
    assert_eq!(machine.csr(MEPC), 0);
    assert_ne!(
        machine.csr(MSTATUS) & (1 << MSTATUS_SPIE),
        0,
        "sie moved to spie"
    ...
    assert_eq!(machine.reg(A0), 1, "the supervisor handler ran");
}

#[test]
fn a_trap_in_machine_mode_is_never_delegated() {
    let machine = prog(&[ecall(), csrrw(ZERO, 0x305, ZERO), addi(A0, ZERO, 1)])
        .csr(MEDELEG, !0)
        .csr(STVEC, DRAM_BASE + 0x800)
        .csr(MTVEC, DRAM_BASE + 4)
        .expect(Exception::IllegalInstruction(0));
    assert_eq!(machine.mode(), Mode::Machine);
    assert_eq!(machine.csr(MCAUSE), 11, "environment call from m-mode");
    assert_eq!(machine.csr(SCAUSE), 0, "delegation cannot lower the mode");
}
...
fn mpp_cannot_be_left_holding_the_mode_that_does_not_exist() {
    let machine = prog(&[csrrw(ZERO, 0x300, T0), csrrs(A0, 0x300, ZERO)])
...
fn a_fault_below_machine_mode_still_reports_its_value() {
    let machine = prog(&[lw(A0, T0, 0), csrrw(ZERO, 0x305, ZERO)])
        .mode(Mode::Supervisor)
        .reg(T0, 2)
        .csr(MTVEC, DRAM_BASE + 4)
        .expect(Exception::IllegalInstruction(0));
    assert_eq!(machine.csr(MCAUSE), 5, "load access fault");
    assert_eq!(machine.csr(MTVAL), 2, "the address it tried to read");
    assert_eq!(machine.csr(MSTATUS) & MSTATUS_MPP, mpp(Mode::Supervisor));
}

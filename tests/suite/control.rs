...
fn branch_taken(inst: u32, lhs: u64, rhs: u64) -> bool {
    let machine = prog(&[inst, addi(T2, ZERO, 1), jal(ZERO, 8), addi(T2, ZERO, 2)])
        .reg(T0, lhs)
        .reg(T1, rhs)
        .run();
    match machine.reg(T2) {
        1 => false,
        2 => true,
...
fn a_backward_branch_closes_a_loop() {
    ...
    .run();
    assert_eq!(machine.reg(T2), 5);
    assert_eq!(machine.reg(T1), 0);
}

#[test]
fn jal_links_the_following_instruction() {
    let machine = run(&[jal(RA, 8), addi(T0, ZERO, 1), addi(T1, ZERO, 2)]);
    assert_eq!(
        machine.reg(RA),
        DRAM_BASE + 4,
        "the return address is pc + 4"
    );
    assert_eq!(machine.reg(T0), 0, "the skipped instruction did not run");
    assert_eq!(machine.reg(T1), 2);
}
...
fn jalr_reads_its_base_before_writing_the_link() {
    ...
    .run();
    assert_eq!(machine.reg(T1), 0, "the skipped instruction did not run");
    assert_eq!(machine.reg(T2), 2);
    assert_eq!(
        machine.reg(RA),
        DRAM_BASE + 8,
...
fn jalr_clears_the_low_bit_of_its_target() {
    ...
    let machine = prog(&[jalr(RA, T0, 1), addi(T1, ZERO, 1), addi(T2, ZERO, 2)])
        .reg(T0, DRAM_BASE + 8)
        .run();
    assert_eq!(machine.reg(T1), 0, "the skipped instruction did not run");
    assert_eq!(machine.reg(T2), 2);
